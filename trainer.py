...
class ScopeTrainer(object):
    ...
    def valid_epoch(self, data_features, is_bert):
            ...
            if is_bert:
                if param.matrix:
                    if param.multi:
                        ...
                        scope_pred, scope_tar = util.handle_eval_multi(temp_scope_pred, scopes, temp_cue_pred, cues)
                        print()
                    else:
        ...
        return {'1': {'f1-score':0,'precision':0,'recall':0,'support':0}}, 0,0
...
class MultiScopeTrainer(object):
    def __init__(self, model, n_gpu, logger, criterion, optimizer, lr_scheduler,
                 label2id, gradient_accumulation_steps, grad_clip=0.0,early_stopping=None,
                 resume_path=None, training_monitor=None, model_checkpoint=None):
    ...
    def valid_epoch(self, data_features, is_bert):
        ...
        for step, f in enumerate(data_features):
            num_labels = param.label_dim
            master_input_ids = f[6].to(self.device)
            master_padding_masks = f[1].to(self.device)
            scopes = f[2]
            input_lens = f[3]
            cues = f[4]
            master_subword_masks = f[5].to(self.device)
            bs = f[0].size(0)
            scopes_matrix = f[-1].to(self.device)
            self.model.eval()
            with torch.no_grad():
                pad_matrix = []
                for i in range(bs):
                    tmp = master_padding_masks[i].clone()
                    tmp = tmp.view(param.max_len, 1)
                    tmp_t = tmp.transpose(0, 1)
                    mat = tmp * tmp_t
                    pad_matrix.append(mat)
                pad_matrix = torch.stack(pad_matrix, 0)
                active_padding_mask = pad_matrix.view(-1) == 1
                scope_logits = self.model(master_input_ids, master_padding_masks)[0]
                
                logits_masked = scope_logits.view(-1, num_labels)[active_padding_mask]
                target_masked = scopes_matrix.view(-1)[active_padding_mask]
                loss = self.criterion(logits_masked, target_masked)

            valid_loss.update(val=loss.item(), n=master_input_ids.size(0))

            if is_bert:
                tmp_scope_pred, tmp_cue_pred = util.multi_matrix_decode_toseq(scope_logits, pad_matrix)                    
                scope_pred = []
                scope_tar = []
                for i in range(bs):
                    pred, tar = pack_subword_pred(tmp_scope_pred[i].detach().cpu().unsqueeze(0), scopes[i].detach().cpu().unsqueeze(0),
                                                  master_subword_masks[i].detach().cpu().unsqueeze(0), master_padding_masks[i].cpu().unsqueeze(0))
                    scope_pred.append(pred[0])
                    scope_tar.append(tar[0])
            else:
                scope_pred = scope_logits.argmax()
            ...
            if param.dataset_name == 'sherlock':
                ...
                new_pred = []
                new_tar = []
                for i, seq in enumerate(master_input_ids):
                    text_seq = data_features.tokenizer.convert_ids_to_tokens(seq)
                    text_string = data_features.tokenizer.decode(seq)
                    new_pred.append(util.postprocess_sher(seq, scope_pred[i], cues[i], master_subword_masks[i], input_lens[i], text_seq, text_string))
                    new_tar.append(util.postprocess_sher(seq, scope_tar[i], cues[i], master_subword_masks[i], input_lens[i], text_seq, text_string))
            ## TODO: handle the scope for mis-predicted cues
                for i1, sent in enumerate(new_pred):
                    for i2, _ in enumerate(sent):
                        wrap_scope_pred.append(new_pred[i1][i2])
                        wrap_scope_tar.append(new_tar[i1][i2])
    ...
    def train_epoch(self, data_loader, valid_data):
        ...
        for step, batch in enumerate(data_loader):
            self.model.train()
            input_ids, master_padding_masks, scopes, input_len, cues, master_subword_masks, master_input_ids, master_mat = batch
            master_input_ids = master_input_ids.to(self.device)
            master_padding_masks = master_padding_masks.to(self.device)
            master_subword_masks = master_subword_masks.to(self.device)
            scopes_matrix = master_mat.to(self.device)
            bs = batch[0].size(0)
            pad_matrix = []
            for i in range(bs):
                tmp = master_padding_masks[i].clone()
