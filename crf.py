...
class CRF(nn.Module):
    def __init__(self,tagset_size,tag_dictionary,device,is_bert=None):
        ...
        self.tag_dictionary[self.START_TAG] = -1
        self.tag_dictionary[self.STOP_TAG] = -2
    ...
    def _forward_alg(self, feats, lens_):
        feats = feats.cpu()
        lens_ = lens_.detach().cpu()
        mask = torch.ones([lens_.size(0), param.max_len, param.label_dim])
        for i, sent in enumerate(lens_):
            for ii in range(sent):
                mask[i][ii] = torch.zeros(param.label_dim)
        mask = mask.to(dtype=torch.bool)
        feats = feats.masked_fill(mask, 0)
    ...
    def calculate_loss(self, scores, tag_list, lengths):
        return self._calculate_loss_old(scores, tag_list, lengths)

    def _calculate_loss_old(self, features, tags, lengths):
